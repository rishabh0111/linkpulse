terraform {
  ...
  # Same partial configuration as envs/aws, different key. The key is what keeps the two
  # state files apart, and that separation is what makes destroying this one safe.
  backend "s3" {
    key          = "aws-burst/terraform.tfstate"
    encrypt      = true
    use_lockfile = true
  }
}
