terraform {
  ...
  #
  # LocalStack is ephemeral: its data lives in a container that `mise run clean` deletes.
  # State describing resources inside it is worthless the moment the container is gone,
  # so there is nothing to protect and nothing to share. Storing it in LocalStack's own
  # S3 would be storing the map inside the territory.
  backend "local" {}
}
